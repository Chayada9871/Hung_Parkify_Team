...
import { FaCalendarAlt, FaPen, FaSearch, FaClock, FaMoneyBillWave } from "react-icons/fa";
import { useRouter } from "next/navigation";
import { Toaster, toast } from "react-hot-toast";

// Utility: convert UTC time to Thai time (UTC+7)
  ...
  return (
    <div className="flex flex-col h-screen bg-white">
      ...
      <div className="relative flex-grow overflow-y-auto p-6">
        ...
        <h1 className="text-2xl font-bold text-black text-left w-full px-6 mt-16 py-4">Reservations</h1>
            ...
            Object.entries(reservationsByDate).map(([date, reservations]) => (
              <div key={date}>
                <div className="flex items-center mb-2">
                  <span className="bg-black text-white px-3 py-1 rounded-full text-sm">{date}</span>
                </div>

                {reservations.map((reservation) => (
                  <div key={reservation.reservation_id} className="p-4 bg-white rounded-lg shadow-md mb-4">
                    <div className="flex justify-between items-center">
                      <div>
                        <h2 className="text-lg font-semibold text-black">Reservation ID: {reservation.reservation_id}</h2>
                        <p className="text-sm text-gray-500">User ID: {reservation.user_id}</p>
                        <div className="flex items-center text-gray-700 mt-2">
                          <FaClock className="mr-2" />
                          <span>{formatThaiTime(reservation.start_time)} - {formatThaiTime(reservation.end_time)}</span>
                        </div>
                        <div className="flex items-center text-gray-700 mt-2">
                          <FaMoneyBillWave className="mr-2" />
                          <span>Total Price: {reservation.total_price ? Number(reservation.total_price).toFixed(2) : "0.00"}฿</span>
                        </div>
                      </div>
