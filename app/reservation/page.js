...
const ParkingDetail = () => {
  ...
  useEffect(() => {
    const fetchUserCars = async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error(data.error || "Error fetching cars");
  ...
  return (
    <div className="w-full h-full bg-gray-100 flex flex-col items-center py-8">
      <Toaster />
      <div className="w-full max-w-screen-xl bg-white shadow-lg rounded-lg p-6 lg:p-12 relative">
        <button
        ...
        <img
          src={parkingDetails.carpark || '/images/Map.png'}
          alt="Parking Lot"
        ...
        <div className="flex flex-row justify-between items-center mb-6 space-x-4">
          ...
          <span className="bg-green-500 text-white font-bold px-4 py-2 rounded-full text-base lg:text-xl">{parkingDetails.price} THB</span>
        </div>
